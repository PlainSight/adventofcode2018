...
function visitRoom(x, y, entrance) {
	...
	if (!room) {
		...
		if (smallestValue < room.value) {
			room.value = smallestValue;
		}
	}
}
	...
	while(true) {
		...
		switch(input[index]) {
			...
			case '(':
				...
				break;
			case '|':
				var co = positionStack[positionStack.length-1];
				x = co.x;
				y = co.y;
				index++;
				break;
...
console.log('max doors: ' + maxDoors);
console.log('over 1000 doors: ' + over1000Doors);
